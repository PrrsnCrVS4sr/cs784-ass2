...
namespace rt
{
	...
	public:
		/// Constructor
		plane_t(material_t* _mat);
		/// Constructor
		plane_t(material_t* _mat, Eigen::Vector3d _Q, Eigen::Vector3d _u, Eigen::Vector3d _v);
		/// Destuctor
		virtual ~plane_t();

		/// Returns the mandatory object name
		std::string get_name(void) const { return std::string("plane"); }
